...
class Flip(val cli: CliApp) {
  ...
  def syncToMegarepoMessage(): String =
    s"git-flip: sync to $megarepoName ${megarepoCommit()}"
  lazy val flipBranchName: String = {
    s"${System.getProperty("user.name")}/git-flip/$flipRepoName/$minirepoBranch"
  }
  lazy val flipRepoName: String = {
    git.readText.linesIterator.toList
      ...
      .getOrElse {
        sys.error(s"git-flip is not installed: $git")
      }
  }
  ...
  def requireInstalled(): Boolean = {
    if (Files.isRegularFile(git) && Files.isDirectory(gitflip)) {
      false
    } else {
