...
abstract class BaseSuite extends munit.FunSuite {
  ...
  val repo = new Fixture[Path]("repo") {
    ...
    override def beforeAll(): Unit = {
      p = Files.createTempDirectory("git-mini")
      if (Properties.isMac && p.startsWith("/var")) {
        ...
        // `/var` path, causing issues in how git-mini relativizes include
        // paths. This line ensures that git-mini always uses the same path as
    ...
    override def afterAll(): Unit = {
      val exit = DeleteVisitor.deleteRecursively(p, new Flip(GitMini.app))
